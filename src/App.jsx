...
function AnimatedValue({value}){
  const [shown,setShown]=useState(value); const match=value.match(/^(.*?)([\d.]+)(.*)$/);
  useEffect(()=>{if(!match||window.matchMedia("(prefers-reduced-motion: reduce)").matches){setShown(value);return}const target=Number(match[2]),decimals=(match[2].split(".")[1]||"").length,start=performance.now(),duration=760;let frame;const tick=now=>{const p=Math.min((now-start)/duration,1),e=1-Math.pow(1-p,3),n=target*e;setShown(`${match[1]}${decimals?n.toFixed(decimals):Math.round(n)}${match[3]}`);if(p<1)frame=requestAnimationFrame(tick)};frame=requestAnimationFrame(tick);return()=>cancelAnimationFrame(frame)},[value]);
  return shown;
}
function Metric({icon:Icon,label,value,change}){return <article className="metric"><div className="metric-icon"><Icon size={21} weight="duotone"/></div><div><span>{label}</span><strong><AnimatedValue value={value}/></strong><small><TrendUp size={14} weight="bold"/>{change}</small></div></article>}

export function App(){
  const [lang,setLang]=useState("en"),[period,setPeriod]=useState(0),[menu,setMenu]=useState(false),[submitted,setSubmitted]=useState(false),[toast,setToast]=useState(false),[scrollProgress,setScrollProgress]=useState(0),[activeId,setActiveId]=useState("overview"),[scrolled,setScrolled]=useState(false);
  ...
  useEffect(()=>{const update=()=>{const max=document.documentElement.scrollHeight-innerHeight;setScrollProgress(max>0?Math.min(scrollY/max*100,100):0);setScrolled(scrollY>24);let current="overview";for(const id of ids){const el=document.getElementById(id);if(el&&el.getBoundingClientRect().top<=180)current=id}setActiveId(current)};update();addEventListener("scroll",update,{passive:true});return()=>removeEventListener("scroll",update)},[]);
  useEffect(()=>{const els=[...document.querySelectorAll("[data-reveal]")];if(matchMedia("(prefers-reduced-motion: reduce)").matches){els.forEach(el=>el.classList.add("is-visible"));return}const observer=new IntersectionObserver(entries=>entries.forEach(entry=>{if(entry.isIntersecting){entry.target.classList.add("is-visible");observer.unobserve(entry.target)}}),{threshold:.13,rootMargin:"0px 0px -60px"});els.forEach(el=>observer.observe(el));return()=>observer.disconnect()},[lang]);
  ...
  return <div className="app" dir={rtl?"rtl":"ltr"} lang={lang}>
    ...
    <header className={`site-header ${scrolled?"is-scrolled":""}`}><div className="shell header-inner"><Logo t={t}/><nav className="desktop-nav">{t.nav.map((n,i)=><button key={n} className={activeId===ids[i]?"active":""} aria-current={activeId===ids[i]?"page":undefined} onClick={()=>go(ids[i])}>{n}</button>)}</nav><div className="header-actions"><button className="lang" onClick={()=>setLang(rtl?"en":"ar")}><Globe size={18}/>{rtl?"EN":"العربية"}</button><button className="header-cta" onClick={()=>go("contact")}>{t.primary}<ArrowRight size={17} className="directional"/></button><button className="mobile-menu" aria-label="Menu" onClick={()=>setMenu(!menu)}>{menu?<X/>:<List/>}</button></div></div>{menu&&<nav className="mobile-nav">{t.nav.map((n,i)=><button key={n} className={activeId===ids[i]?"active":""} onClick={()=>go(ids[i])}>{n}</button>)}</nav>}</header>
    <main>
      <section className="hero" id="overview"><img src="/assets/riyadh-data-skyline.webp" alt="Riyadh skyline with subtle analytical data grid"/><div className="hero-shade"/><div className="shell hero-content"><div className="hero-demo-pill"><Lightning size={14} weight="fill"/>{rtl?"معرض أعمال تفاعلي":"Interactive portfolio experience"}</div><div className="eyebrow"><span/>{t.heroEyebrow}</div><h1>{t.heroTitle}</h1><p>{t.heroText}</p><div className="hero-actions"><button className="button primary" onClick={()=>go("contact")}>{t.primary}<ArrowUpRight size={19} className="directional"/></button><button className="button secondary" onClick={()=>go("dashboard")}><ChartLineUp size={19}/>{t.secondary}</button></div><div className="trust-row">{t.trust.map(x=><span key={x}><CheckCircle size={17} weight="fill"/>{x}</span>)}</div><button className="scroll-cue" onClick={()=>go("services")} aria-label={rtl?"استكشف الخدمات":"Explore services"}><MouseSimple size={17}/><span>{rtl?"مرر للاستكشاف":"Scroll to explore"}</span></button></div></section>

      <section className="section services" id="services"><div className="shell"><div className="section-heading reveal" data-reveal><div><span className="kicker">CAPABILITIES</span><h2>{t.servicesTitle}</h2></div><p>{t.servicesText}</p></div><div className="service-grid">{t.services.map(([title,text],i)=>{const Icon=serviceIcons[i];return <article className="service reveal" data-reveal style={{"--delay":`${i*70}ms`}} key={title}><span className="service-num">0{i+1}</span><span className="service-icon"><Icon size={30} weight="duotone"/></span><h3>{title}</h3><p>{text}</p><button onClick={()=>go("contact")}>{rtl?"ناقش مشروعك":"Discuss your project"}<ArrowRight size={16} className="directional"/></button></article>})}</div></div></section>

      <section className="section dashboard-section" id="dashboard"><div className="shell"><div className="dashboard-head reveal" data-reveal><div><span className="demo-tag">{t.demo}</span><h2>{t.dashboardTitle}</h2><p>{t.dashboardText}</p></div><div className="dashboard-actions"><label>{t.period}<select value={period} onChange={e=>setPeriod(Number(e.target.value))}>{t.periods.map((p,i)=><option value={i} key={p}>{p}</option>)}</select></label><button className="button export" onClick={download}><DownloadSimple size={18}/>{t.export}</button></div></div><div className="dashboard-frame reveal" data-reveal><div className="demo-note"><ShieldCheck size={17}/>{t.demoLong}</div><div className="metric-grid"><Metric icon={CurrencyCircleDollar} label={t.kpis[0]} value={metrics[0][0]} change={metrics[0][1]}/><Metric icon={ChartBar} label={t.kpis[1]} value={metrics[1][0]} change={metrics[1][1]}/><Metric icon={Gauge} label={t.kpis[2]} value={metrics[2][0]} change={metrics[2][1]}/><Metric icon={TrendUp} label={t.kpis[3]} value={metrics[3][0]} change={metrics[3][1]}/></div>
      ...
      <section className="section cases" id="cases"><div className="shell"><div className="section-heading reveal" data-reveal><div><span className="kicker">CASE STUDIES</span><h2>{t.casesTitle}</h2></div><p>{t.casesText}</p></div><div className="case-grid">{t.cases.map(([title,text,a,b],i)=>{const Icon=caseIcons[i];return <article className="case reveal" data-reveal style={{"--delay":`${i*80}ms`}} key={title}><div className="case-top"><Icon size={28} weight="duotone"/><span>{t.demo}</span></div><h3>{title}</h3><p>{text}</p><div className="case-stats"><span>{a}</span><span>{b}</span></div></article>})}</div><div className="process"><h3>{t.processTitle}</h3><div>{t.process.map((p,i)=><span key={p}><b>{i+1}</b>{p}{i<4&&<i/>}</span>)}</div></div></div></section>

      <section className="contact" id="contact"><div className="shell contact-grid"><div className="contact-copy reveal" data-reveal><span className="kicker light">{t.contactEyebrow}</span><h2>{t.contactTitle}</h2><p>{t.contactText}</p><div className="contact-detail"><EnvelopeSimple size={22}/><span><small>Email</small><strong>hello@baaddata.example</strong></span></div><div className="contact-detail"><Phone size={22}/><span><small>{rtl?"الموقع":"Location"}</small><strong>{rtl?"مكة المكرمة، السعودية":"Makkah, Saudi Arabia"}</strong></span></div></div><div className="form-card reveal" data-reveal>{submitted?<div className="success"><CheckCircle size={50} weight="fill"/><h3>{t.sentTitle}</h3><p>{t.sentText}</p><button className="button primary" onClick={()=>setSubmitted(false)}>{t.reset}</button></div>:<form onSubmit={e=>{e.preventDefault();setSubmitted(true)}}><div className="form-row"><label>{t.name}<input required placeholder={rtl?"الاسم الكامل":"Full name"}/></label><label>{t.email}<input required type="email" placeholder="name@company.com"/></label></div><div className="form-row"><label>{t.company}<input required placeholder={rtl?"اسم الشركة":"Company name"}/></label><label>{t.budget}<select required defaultValue=""><option value="" disabled>{rtl?"اختر النطاق":"Select range"}</option><option>SAR 2,000–5,000</option><option>SAR 5,000–10,000</option><option>SAR 10,000+</option></select></label></div><label>{t.need}<select required defaultValue=""><option value="" disabled>{rtl?"اختر الخدمة":"Select a service"}</option>{t.needs.map(n=><option key={n}>{n}</option>)}</select></label><label>{t.message}<textarea required rows="4" placeholder={rtl?"صف البيانات المتوفرة والنتيجة التي تحتاجها...":"Describe the data you have and the outcome you need..."}/></label><button className="button primary form-submit" type="submit">{t.submit}<ArrowRight size={19} className="directional"/></button></form>}</div></div></section>
    </main>
    {scrolled&&activeId!=="contact"&&<button className="floating-cta" onClick={()=>go("contact")}><Briefcase size={19} weight="duotone"/><span>{t.primary}</span><ArrowUpRight size={17} className="directional"/></button>}
